...
function ProductCard({ product }) {
  ...
  const buyProduct = async (product) => {
    ...
    try {
      ...
      await axios
        .put("https://e-com-back.onrender.com/buyproducts", data)
            ...
            <div style={{ backgroundColor: "aliceblue" }}>
              <Card.Body className="description_font">
                ...
                <Card.Text className="cost">
                  <i className=" fa-sharp fa-solid fa-indian-rupee-sign rup_siz"></i>
                  {product.cost}
                </Card.Text>
              </Card.Body>

              <div className="d-flex justify-content-evenly mb-4 description_font">
                {/* <NavLink to="directcheckout"> */}
                  <Button
                    className="btn-buy-cart"
                    onClick={() => buyProduct(product)}
                  >
                    <i
                      className=" fa-brands fa-buy-n-large fa-shake rup_siz"
                      style={{ color: "0d0d0d", fontSize: "19px" }}
                    ></i>
                    BUY
                  </Button>
                {/* </NavLink> */}
                <span
                  className="btn-buy-cart border-0 "
                  style={{ backgroundColor: "aliceblue", color: "#039703" }}
                >
                  <i
                    class="fa-solid fa-bullhorn fa-beat"
                    style={{ color: "#0d0d0d", marginRight: "4px" }}
                  ></i>
                  <img src={offer} className="offer_img" />
                  {totalDiscount}
...
const All_Product = () => {
  ...
  useEffect(() => {
    axios
      .get("https://e-com-back.onrender.com/api/products")
      .then((items) => setGetProduct(items.data))
